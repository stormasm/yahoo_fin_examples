...
def tickers_ibovespa(include_company_data=False):
    ...
    table = pd.read_html(
        "https://pt.wikipedia.org/wiki/Lista_de_companhias_citadas_no_Ibovespa"
    )[0]
...
def tickers_ftse100(include_company_data=False):
    ...
    table = pd.read_html(
        "https://en.wikipedia.org/wiki/FTSE_100_Index", attrs={"id": "constituents"}
    )[0]
...
def tickers_ftse250(include_company_data=False):
    ...
    table = pd.read_html(
        "https://en.wikipedia.org/wiki/FTSE_250_Index", attrs={"id": "constituents"}
    )[0]
